...
__global__ void gpu_pmult(   
                            word64* c1_a, word64* c1_b, 
                            word64* c2, 
                            int log_degree,
                            int degree, int length,
                            word64* gbarret_ratio, word64* gbarret_k,
                            word64* gprimes,
                            word64* d0, word64* d1
                            ) 
{
    ...
    for(int i=0; i<length; i++)
    {
        ...
        mult_64_64_128(c1_a[now], c2[now], res1);
        mult_64_64_128(c1_b[now], c2[now], res2);
        auto gbarret_ratiox=gbarret_ratio[i];
        auto gbarret_kx=gbarret_k[i];
        out1=barrett_reduction_128_64(res1, prime , gbarret_ratiox
                                    ,gbarret_kx);
        out2=barrett_reduction_128_64(res2, prime , gbarret_ratiox
                                    ,gbarret_kx);                            
        d0[now]=out1;
        d1[now]=out2;
        now+=degree;
...
int main(int argc, char const* argv[])
{
    ...
    c2   = (word64*)malloc(sizeof(word64) * sumlength);
    ...
    cudaMalloc(&gc2, sizeof(word64) * sumlength);
    ...
    cudaMalloc(&gbarret_ratio, sizeof(word64) * max_num_moduli);
    cudaMalloc(&gbarret_k, sizeof(word64) * max_num_moduli);
    ...
    cudaMemcpy(gc2, c2, sizeof(word64) * sumlength, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(gbarret_k, barret_k_64, sizeof(word64) * max_num_moduli, cudaMemcpyHostToDevice);
